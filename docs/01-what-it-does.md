...
Griffiths factor. Drought is a property of a district, not a 15 km cell, so it is worked out over the
hexagon and its ring — about 45 km across — from the Bureau stations inside those seven hexagons (or
the nearest within 75 km).

Starting an area fetches only the days the stations do not cover: running for 30 days and needing 365,
it asks Open-Meteo's archive for the year behind the ledger and nothing else. After that it is free:
every day after 9:10 am local, each area whose last complete day is behind the calendar is stepped
forward from the ledger, exactly once, and the state is kept on the hexagon so a quiet area picks up
where it left off. A reading with no drought state has no fire index.
